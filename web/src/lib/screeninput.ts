...
// Что делать с отказом ручки ввода. Развилка вынесена из компонента, потому что цена
// ошибки здесь — не сообщение, а мёртвое управление до конца сеанса.
//
// 🔴 Поймано в поле: 409 «стрим ещё не открыт» приходил в первые 3.5 с сеанса — пока
// приглашение едет на устройство, а захватчик поднимается. Панель схлопывала его с 409
// «управление возвращено» в терминальный исход и гасила управление НАВСЕГДА, хотя сервер
// его не отзывал (в БД control=t, control_returned_at=NULL). Оператор видел картинку с
// мёртвым вводом; лечилось только новым сеансом.
//
// Поэтому: временное и окончательное — разные ответы, и различает их СЕРВЕР кодом, а не
// панель угадыванием по телу.
export type InputOutcome =
  | "retry" // 503 — повторить: затор у агента либо стрим ещё не открыт
  | "dead" // 409/410 — управления в этом сеансе больше не будет
  | "unknown" // остальное — показать текст ошибки, управление не трогать

export function inputOutcome(status: number | undefined): InputOutcome {
  if (status === 503) return "retry"
  if (status === 409 || status === 410) return "dead"
  return "unknown"
}
