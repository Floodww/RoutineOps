...
## Две роли БД: сервер и миграции ходят под разными

В `.env.prod` два DSN, и это не дублирование:

| Переменная | Роль | Зачем |
|---|---|---|
| `DATABASE_DSN` | `mdm_app` | под ней ходит **сервер**. `NOSUPERUSER NOBYPASSRLS` (миграция `049`) — только такая роль подчиняется `RLS FORCE` из `046`, то есть изоляции тенантов |
| `MIGRATION_DSN` | `mdm` | под ней катятся **миграции** (DDL). Это `POSTGRES_USER` из compose, владелец БД |

**Почему это важно.** Владелец БД обходит RLS всегда. Пока сервер ходил под ним,
политики изоляции висели на всех таблицах и не работали: любой запрос видел строки всех
тенантов. Проверка «политики на месте» этого не показывает — она зелёная в обоих случаях.
Отличает рабочую изоляцию от декоративной ровно одно:

```sql
SELECT rolsuper, rolbypassrls FROM pg_roles WHERE rolname = current_user;  -- ожидается f, f
```

**Ничего делать руками не нужно.** `install.sh` и `update.sh` вызывают
`scripts/env-db-roles.sh`, который приводит раскладку к разделённой (и на свежей
установке, и на приехавшей со старой одной ролью). Пароль роли `mdm_app` ставит
`scripts/migrate.sh` — после наката `049`, когда роль уже существует; берётся он из
самого `DATABASE_DSN`, поэтому рассинхрон «в базе один пароль, у сервера другой»
невозможен. Пароль, выставленный вручную, не перетирается: если вход под ролью уже
работает, шаг пропускается.

Гейт: `migrate` **отказывается** катить DDL, если `MIGRATION_DSN` не задан, а сервер уже
переведён на `mdm_app` — иначе миграции упали бы на `permission denied` посреди прогона,
записав часть версий в `schema_migrations`. Регрессия: `scripts/test-install-db-role.sh`
(поднимает настоящий Postgres и проверяет оба исхода).
