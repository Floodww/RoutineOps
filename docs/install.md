...
| `TRUSTED_PROXIES` | приватные и loopback-диапазоны | С каких адресов верить заголовку `X-Real-IP` / `X-Forwarded-For` при определении адреса клиента. Список CIDR или голых адресов через запятую |

> **`TRUSTED_PROXIES` нужен, только если ваш обратный прокси стоит на публичном
> адресе** (отдельный хост, облачный балансер). В штатной установке nginx живёт рядом
> в compose-сети, его адрес приватный, и дефолта достаточно.
>
> Зачем это вообще: лимиты на попытки входа считаются по адресу клиента, а из-за
> прокси сервер видит адрес прокси. Без восстановления настоящего адреса все клиенты
> попадают в один общий счётчик — перебор с одной машины съедает лимит и роняет вход
> остальным. Если прокси публичный и не перечислен здесь, будет ровно это.
>
> Заголовку верят **только** от перечисленных адресов. Запрос с любого другого адреса
> со своим `X-Real-IP` заголовок не обманет — иначе лимит обходился бы новым значением
> на каждый запрос.
...
> Этот токен действует **24 часа** и одноразовый — он привязан к конкретному
> устройству. Если истёк или сгорел — нажмите «Перерегистрировать» на странице
> устройства. Подробнее — [enrollment.md](enrollment.md).
>
> **Для парка машин этот путь не нужен.** Заводить устройства заранее по одному не
> требуется: есть массовый токен — один на всю партию, срок по умолчанию 7 дней,
> устройство создаётся само при первом подключении. См.
> [«Массовая установка на парк»](#массовая-установка-на-парк-bulk-энроллмент).
...
## Массовая установка на парк (bulk-энроллмент)

Раскатка через групповые политики AD, KSC, Ansible или любой другой инструмент
доставки. **Заранее заводить устройства в интерфейсе не нужно** — сервер создаёт
запись сам, когда агент впервые придёт с массовым токеном.

### Чем массовый токен отличается от одноразового

| | Одноразовый | Массовый (bulk) |
|---|---|---|
| Привязан к устройству | да, `device_id` назначен заранее | нет |
| Число установок | одна | лимит `max_uses` либо без ограничения |
| Срок жизни | 24 часа | **7 дней по умолчанию**, задаётся при выпуске |
| Устройство в списке | до энроллмента, как `pending` | создаётся в момент подключения |
| Очередь одобрения | нет | **есть, включена по умолчанию** |
| Целевая группа | назначается вручную потом | вшивается в токен |

Обе разновидности — в свободной редакции.

### 1. Выпустите массовый токен

Интерфейс → **«Энроллмент»** → **«Массовые токены»** → **«Выпустить токен»**.
Поля: группа, **лимит использований** (пусто — без ограничения), **срок жизни в
часах** (пусто — 168, то есть неделя) и **«Требовать одобрения»**.

Панель сразу отдаёт готовую команду установки со всеми свойствами, включая
`ca_sha256` — отдельно считать хеш CA не нужно.

Для автоматизации есть REST:

```
POST /api/v1/enrollment-tokens/bulk
{
  "group_id": "<uuid или пусто>",
  "max_uses": 200,            // отсутствует или null — без ограничения
  "ttl_hours": 336,           // 0 или отсутствует — 168 (7 дней)
  "require_approval": true    // отсутствует — true
}
→ 201 { "enrollment_token": "...", "expires_at": "...", "ca_sha256": "...", "require_approval": true }
```

> Выпуск токена — операция под `requireHuman`: она даёт машинам вход в парк, поэтому
> сервисным API-токеном её не выполнить, только живым администратором из панели или
> под его сессией. Это осознанное ограничение, а не недоработка.

### 2. Раскатайте установщик

Токен один и тот же на всю партию — меняется только способ доставки.

#### Windows через групповые политики

MSI универсален и ставится тихо:

```
msiexec /i RoutineOps-agent.msi /qn ^
  ENROLL_URL="https://mdm.example.com/api/v1/enroll" ^
  ENROLL_TOKEN="<массовый токен>" ^
  CA_URL="https://mdm.example.com/ca.crt" ^
  CA_SHA256="<ca_sha256 из ответа>" ^
  SERVER_ADDR="mdm.example.com:50051"
```

> 🔴 **Назначение MSI через «Установка программ» в GPO свойства не передаёт.**
> Политика умеет ставить пакет, но не умеет прокидывать `ENROLL_TOKEN` и остальные
> четыре свойства, а без них установка прервётся на Launch-условии. Рабочих путей два:
>
> - **скрипт запуска компьютера** (Computer Configuration → Windows Settings →
>   Scripts → Startup) с приведённой выше строкой `msiexec` — самый прямой;
> - **трансформ `.mst`** с прописанными свойствами, приложенный к пакету в политике
>   установки, — если раскатка обязана идти именно через «Установку программ».

#### KSC и прочие системы доставки

KSC задаёт параметры установки штатно — вставьте те же пять свойств в параметры
инсталляционного пакета. То же касается SCCM/Intune и самописных раскаток: требуется
лишь возможность передать msiexec аргументы.

#### Linux и macOS

Тот же токен, обычная команда энроллмента — она не различает виды токенов:

```sh
sudo /usr/local/bin/RoutineOps-agent enroll -install-service \
  -enroll-url https://mdm.example.com/api/v1/enroll -server mdm.example.com:50051 \
  -token <массовый токен> \
  -ca-url https://mdm.example.com/ca.crt -ca-sha256 <ca_sha256 из ответа>
```

### 3. Одобрите подключившихся

При `require_approval` (по умолчанию) машины не получают доступ сразу: они встают в
**«Энроллмент» → «Очередь одобрения»** со статусом «Ожидает одобрения». Сертификат
им уже выдан, но полномочий нет, пока администратор не одобрит — поштучно или кнопкой
«Одобрить все».

> Одобрение снимает ограничение целиком, включая выполнение скриптов, и применяется
> ко всем, кто стоит в очереди на момент подтверждения, — в том числе к тем, кто
> подключился, пока вы читали список.
>
> Отклонение **терминально**: вернуть машину через одобрение нельзя, её придётся
> энролить заново.

Выключать одобрение (`require_approval: false`) осмысленно только там, где сам канал
доставки уже доверенный — например, GPO в закрытом домене. Тогда машины подключаются
сразу.

### 4. Закройте окно и проверьте охват

- **Отзыв токена** — кнопка «Отозвать» или `DELETE /api/v1/enrollment-tokens/{id}`.
  Срабатывает мгновенно; уже заэнролленных устройств отзыв не касается.
- **«Не подключились»** — раздел показывает одобренные устройства, от которых не
  пришло ни одного heartbeat. По нему и видно, до кого раскатка не доехала.
...
| `msiexec` прерывается с текстом «уже зарегистрирован» | Продукт числится установленным, и энроллмент в этом прогоне не выполнится (режим обслуживания) | Повторить ту же команду с `REINSTALL=ALL REINSTALLMODE=vomus`. Если агента на диске нет — сперва снять запись: `msiexec /x {ProductCode} /qn` |
| `msiexec` вернул **0**, а службы и сертификатов нет | Сиротская регистрация установщика: агента снимали процедурой, которая не дёргала `msiexec /x` | [field-troubleshooting.md](field-troubleshooting.md) — снять запись `msiexec /x {ProductCode} /qn` и поставить заново. Установщик с гейтом на этот случай такой прогон не пропускает молча, а прерывается |
