...
- **Инвентаризация** — hostname, ОС, CPU/RAM/диск, IP, серийный номер, версия агента, события процессов, пользователь за консолью, владелец устройства. По каждому приложению: издатель, путь установки, архитектура, идентификатор для снятия и признак «на машину / в профиль пользователя» (на Windows видны и профильные установки — именно ими обходили запрет ПО).
...
- **Удаление ПО** — снятие установленного приложения с устройства прямо из карточки (роль `it_admin`). Селектор цели сервер собирает из своего инвентаря, а не из тела запроса; агент сам выбирает метод снятия (msiexec / dpkg / rpm / …).
- **Блокировка устройства** — полноэкранный overlay с паролем (Windows и macOS), разблокировка работает и офлайн. Снять блокировку может только служба: попытка пометить устройство разблокированным в обход неё возвращает лок на место и уезжает в ИБ отдельным событием.
- **Удалённая перезагрузка** — одной машины или всей группы (окно обслуживания). Сотрудник получает отсрочку на сохранение работы, отсчитывает её сама ОС; панель говорит «запланирована», а не «перезагружено».
- **Владельцы устройств** — карточка человека (ФИО + почта) заводится прямо в карточке устройства. Владельцу НЕ нужен вход в панель и приглашение: аккаунты нужны тем, кто в панель ходит.
- **События и алерты** — `agent_unreachable` (устройство пропало с радаров), запрещённое ПО, несанкционированная установка/изменение настроек, попытка обхода блокировки; группировка по типу со счётчиком неподтверждённых; уведомления в Telegram.
- **Журнал аудита** — все действия администраторов, retention настраивается (`AUDIT_RETENTION_DAYS`, по умолчанию 365 дней).
- **RBAC** — роли `it_admin` (полный доступ) и `viewer` (только чтение); приглашения по email — для тех, кому нужен вход в панель (администраторы, поддержка), а не для владельцев устройств.
- **Self-update агентов** — подписанные ed25519-релизы, проверка sha256 и подписи манифеста, защита от отката версии.
- **Каналы обновлений и канареечная выкатка** — у группы устройств есть канал (`stable` / `beta`), у релиза тоже. Обкатали новую версию на канареечной группе — перевели релиз в `stable`, и его подхватывает весь парк. Экран «Выкатка» показывает, какие версии где стоят.
- **Интерфейс на русском и английском** — переключатель в профиле, язык запоминается; серверные сообщения и подписи действий журнала тоже переведены. Полноту словарей стережёт гейт: пропущенный ключ роняет сборку, а не всплывает у пользователя.
...
- [`docs/openapi.yaml`](./docs/openapi.yaml) — **спецификация REST API** (OpenAPI 3): все ручки панели, роли и коды ответов. Открывается любым просмотрщиком OpenAPI (Swagger UI, Redoc) или читается как есть.
...
- [`docs/ROADMAP.md`](./docs/ROADMAP.md) — что уже готово, что в работе и что осознанно не делается.
- [`CHANGELOG.md`](./CHANGELOG.md) — изменения по выпускам, глазами оператора.
...
| Настройка FileVault из админки (диалог сотруднику в трее, без визита к машине) | ✅ |
| Каталог пользователей (LDAP / Active Directory): синхронизация и авто-привязка владельца устройства | ✅ |
| Вход в панель по паролю каталога (LDAP/AD) | ✅ |
| Уровни критичности алертов и маршрутизация | ✅ |
| Tamper-evident аудит (хеш-цепочка журнала) | ✅ |
| Мультитенантность (отдельные тенанты со своими политиками) | ✅ |
| SSO: OIDC и SAML, оба настраиваются из панели | ✅ |
| MFA / TOTP с кодами восстановления, политика тенанта «требовать второй фактор» | ✅ |
| SCIM-провижининг пользователей из IdP | ✅ |
| Экспорт журнала в SIEM (syslog/CEF, webhook) + архив журнала перед чисткой | ✅ |
| Дашборд соответствия и выгрузка отчёта | ✅ |
| Сканирование установленного ПО на уязвимости (CVE) | ✅ |
| Удалённое подключение к рабочему столу (просмотр, управление, запись сеанса) | 🔄 реализовано; полевая приёмка пройдена на Windows |
| HA / несколько узлов | 📐 спроектировано |
