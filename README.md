...
Подробно с самим функционированием и моделью безопасности вы можете ознакомиться в следующих файлах: [`ARCHITECTURE.md`](./ARCHITECTURE.md) и [`SECURITY.md`](./SECURITY.md).
...
- **Инвентаризация** — hostname, ОС, CPU/RAM/диск, IP, серийный номер, версия агента, список установленного ПО, события процессов, пользователь за консолью, владелец устройства.
...
- **Вывод из эксплуатации** — кнопка в карточке устройства: агент полностью сносит себя (файлы, ключи, автозагрузка, запись установщика). Удаление из инвентаря отзывает сертификат устройства — «воскрешение» пустой записью невозможно.
- **Конфигурация-как-код** — скрипты, политики и группы в YAML: `routineops export` / `apply`, конфигурация парка в git.
- **API-токены** — выпуск и отзыв из панели (роль, срок жизни) для автоматизации через REST API.
...
- [`docs/config-as-code.md`](./docs/config-as-code.md) — группы, скрипты и политики в YAML: `routineops export` / `apply`, конфигурация парка в git.
...
| Каталог пользователей (LDAP / Active Directory): синхронизация и авто-привязка владельца устройства | ✅ |
...
**По вопросам Enterprise-лицензии, багам и предложениям** — пишите на
[charmander020104@gmail.com](mailto:charmander020104@gmail.com) или в Telegram
[@flood_j](https://t.me/flood_j), либо открывайте
[Issue](https://github.com/Floodww/RoutineOps/issues) в этом репозитории.
