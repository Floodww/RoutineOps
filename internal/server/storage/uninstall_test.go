...
func seedSoftware(t *testing.T, db *storage.DB, fp string, item storage.SoftwareItem) {
	t.Helper()
	err := db.UpsertInventory(tenantCtx(), storage.InventoryData{
		CertFingerprint: fp,
...
func activeDeviceWithSoftware(t *testing.T, db *storage.DB, item storage.SoftwareItem) (deviceID string) {
	...
	if err := db.UpdateDeviceStatus(tenantCtx(), tenancy.DefaultTenantID, id, "active"); err != nil {
...
func TestCreateUninstallTask_SelectorComesFromInventory(t *testing.T) {
	...
	task, err := db.CreateUninstallTask(tenantCtx(), deviceID, item.Name, item.UninstallID, "нарушает политику")
	if err != nil {
	...
	again, err := db.CreateUninstallTask(tenantCtx(), deviceID, item.Name, item.UninstallID, "повтор")
	if err != nil {
...
func TestCreateUninstallTask_NotInInventory(t *testing.T) {
	...
	_, err := db.CreateUninstallTask(tenantCtx(), deviceID, "Такого Нет", "{GUID-НЕТ}", "")
...
func TestCreateUninstallTask_NoMethodRefused(t *testing.T) {
	...
	_, err := db.CreateUninstallTask(tenantCtx(), deviceID, "Пользовательский Мессенджер", "user-app", "")
...
func TestCreateUninstallTask_BlockedDeviceRefused(t *testing.T) {
	...
	deviceID := activeDeviceWithSoftware(t, db, item)
	if err := db.UpdateDeviceStatus(tenantCtx(), tenancy.DefaultTenantID, deviceID, "blocked"); err != nil {
		t.Fatalf("blocked: %v", err)
	}
	_, err := db.CreateUninstallTask(tenantCtx(), deviceID, item.Name, item.UninstallID, "")
