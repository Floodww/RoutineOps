...
type Queue struct {
	...
	// mu сериализует слив очереди И Enqueue (запись + enforceLimit). Без второго
	// enforceLimit защищал только запись СВОЕГО вызова (исключение f == newName):
	// свежий файл параллельного Enqueue лежал в списке обычным кандидатом и
	// вытеснялся чужим enforceLimit, а его собственный проход видел очередь уже в
	// пределах max и возвращал nil — ложный durable-успех без ухода в фолбэк.
...
func (q *Queue) Enqueue(kind string, data []byte) error {
	...
	// Под q.mu от записи до enforceLimit: см. комментарий у mu — вне мьютекса
	// параллельный enforceLimit вытеснял чужую свежую запись при durable-ответе nil.
	q.mu.Lock()
	defer q.mu.Unlock()
